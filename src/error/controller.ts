 ...
 */
const sendErrForDev = (err: IAppError, res: Response): void => {
 ...
 * @param res - The Express response object.
 */
const sendErrProduction = (err: IAppError, res: Response): void => {
 ...
 * @returns A new AppError with a descriptive message and status code 400.
 */
const handleMongooseError = (err: IAppError): AppError => {
...
const handleJWTError = (): AppError => {
  return new AppError("Token non valido. Effettuare nuovamente il login!", 401);
};
 ...
 * @returns A new AppError with a descriptive message and status code 401.
 */
const handleJwtTokenExpire = (): AppError => {
  return new AppError("Token scaduto. Effettuare nuovamente il login", 401);
};
 ...
 */
const handleValidationError = (err: IAppError): AppError => {
...
const globalErrorHandler = (
  ...
  if (process.env.NODE_ENV === "development") {
    ...
    if (err.name === "MongooseError") error = handleMongooseError(error);
